...
mask 不要求与原图具有完全相同的像素尺寸。加载器会逐张检查对应关系：例如
`2136×2848` 的 mask 和 `2448×3264` 的原图都是 `4:3`；与图片同步 resize 时，
mask 会用最近邻方式适配，随后继续同步 crop 和 flip。数据集中每张图片可以有不同
尺寸，不需要在 YAML 中填写统一原图大小。只有宽高比明显不同或 batch 对不上时才会
报错，因为这种情况通常代表 mask 来自错误图片，强行拉伸会污染肤色和环境监督。
